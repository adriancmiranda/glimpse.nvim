--- Gerenciamento de placements de imagem em buffers do Neovim.
--- Usa unicode placeholders via Kitty Graphics Protocol.
...
-- Diacríticos para codificar row/col
...
--- Gera o grid de placeholders como linhas reais no buffer.
--- @param buf number
...
local function render_grid(buf, image_id, cols, rows)
	...
	-- Escreve no buffer como linhas reais
	vim.bo[buf].modifiable = true
	vim.api.nvim_buf_set_lines(buf, 0, -1, false, lines)
	vim.bo[buf].modifiable = false
	vim.bo[buf].modified = false

	-- Aplica highlight via extmarks
	for i = 0, #lines - 1 do
...
--- Renderiza uma imagem no buffer.
--- @param buf number
...
--- Registra um placement existente para tracking.
--- @param buf number
--- @param filepath string
...
--- Re-renderiza um placement existente (mantém o antigo até o novo estar pronto).
--- @param buf number
function M.rerender(buf)
	...
	-- Pula se foi criado há menos de 500ms
...
--- Verifica se um buffer tem um placement ativo.
--- @param buf number
--- @return boolean
...
--- Verifica se o placement precisa de re-render (imagem pode ter sido perdida).
--- @param buf number
--- @return boolean
function M.needs_rerender(buf)
	...
	-- Se não tem image_id, ainda está carregando - não re-renderizar
